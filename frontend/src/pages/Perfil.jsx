        ...
        <div className="px-8 pb-8 flex flex-col md:flex-row items-center md:items-start gap-6 relative -mt-16">
          ...
          <div className="flex flex-col items-center z-10">
            <div className="relative group cursor-pointer" onClick={() => fileInputRef.current.click()} title="Alterar foto">
              ...
              <div className="absolute inset-0 bg-slate-900/50 rounded-full opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center text-white transition-all">
                <span className="text-2xl">📷</span>
                <span className="text-xs font-bold">Alterar</span>
              </div>
            </div>
            <input type="file" ref={fileInputRef} className="hidden" accept=".png, .jpg, .jpeg" onChange={handleFotoUpload} />
            {!perfil.foto.includes('ui-avatars') && (
              <button onClick={removerFoto} className="mt-2 text-xs text-slate-400 hover:text-red-500 font-bold">🗑️ Remover foto</button>
            )}
          </div>
          ...
          <div className="mt-4 md:mt-16">
            <button onClick={() => { setFormData(perfil); setIsEditing(true); }} className="bg-white border border-slate-200 px-6 py-2 rounded-xl text-sm font-bold text-slate-700 hover:bg-slate-50 shadow-sm transition">
              Editar Meu Perfil
            </button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl animate-fade-in overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center">
              <h3 className="font-bold text-slate-800 text-xl">Editar Perfil</h3>
              <button onClick={() => setIsEditing(false)} className="text-slate-400 hover:text-slate-600 text-2xl">&times;</button>
            </div>
            <div className="p-6 space-y-4">
              <input type="text" value={formData.nome || ''} onChange={e => setFormData({...formData, nome: e.target.value})} className="w-full p-3 border border-slate-300 rounded-lg outline-none" placeholder="Nome" />
              <input type="text" value={formData.area || ''} onChange={e => setFormData({...formData, area: e.target.value})} className="w-full p-3 border border-slate-300 rounded-lg outline-none" placeholder="Área" />
              <textarea value={formData.bio || ''} onChange={e => setFormData({...formData, bio: e.target.value})} rows="4" className="w-full p-3 border border-slate-300 rounded-lg outline-none" placeholder="Biografia..."></textarea>
            </div>
            <div className="p-6 bg-slate-50 flex justify-end gap-3 border-t border-slate-100">
              <button onClick={() => setIsEditing(false)} className="px-6 py-2 font-bold text-slate-600 hover:bg-slate-200 rounded-lg transition">Cancelar</button>
              <button onClick={salvarEdicao} className="px-6 py-2 bg-blue-600 text-white font-bold rounded-lg shadow-sm hover:bg-blue-700 transition">Salvar Alterações</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isDeletingWork && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white p-8 rounded-2xl text-center max-w-sm shadow-2xl animate-fade-in">
            ...
            <p className="text-slate-500 text-sm mb-8">Esta ação removerá permanentemente o trabalho <b>{workToDelete?.titulo}</b> do seu acervo.</p>
